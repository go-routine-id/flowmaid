...
use crate::model::{
    ...
    FrameKind, Graph, Key, Member, NodeStyle, NoteSide, RelKind, Relation, SeqHead, SeqItem,
            ...
            Some(t) => Err(err(
                ...
                format!(
                    "diagram type '{}' is not supported yet \
                     (supported: flowchart, graph, erDiagram, classDiagram, sequenceDiagram)",
                    t
                ),
            )),
    ...
    for (i, raw) in source.lines().enumerate() {
        ...
        if !header_seen {
            ...
            if let Some(t) = diagram_type(line) {
                let hint = if t == "erDiagram" || t == "classDiagram" || t == "sequenceDiagram" {
